"""
====================================================================
CYCLE 2: 14-DAY PAPER EVALUATION (deployed: 2026-07-10, ends: 2026-07-24)
====================================================================
...
11. DATA-INPUT FIXES (found by the audit trail on scan #1 of cycle 2).
    Three defects were starving the locked strategy of inputs its
    rules require - bug fixes, not strategy changes:
    (a) Kalshi 429s: the scan burst of ~14 market requests was being
        rate-limited, leaving only ~2 markets visible per scan. Fixed
        with request pacing plus retry-with-backoff on 429.
    ...
    (c) No current date in the prompt: the model could not correctly
        ...
        includes the current UTC date/time.
...
# Pace market-data requests so Kalshi's rate limiter doesn't 429 the
# scan burst (14 series fetched back-to-back). Costs ~10s per scan.
KALSHI_REQUEST_SPACING_SEC = 0.7
...
def kalshi_request(method, path, params=None, json_body=None, timeout=15):
    ...
    Retries up to twice on 429 (rate limit) with a short backoff -
    Kalshi throttles bursts, and a one-second pause usually clears it.
    """
    url = BASE_URL + path
    last_exc = None
    for attempt in range(3):
        ...
        try:
            ...
            if r.status_code == 429 and attempt < 2:
                wait = 1.0 + attempt
                log.warning(f"Kalshi 429 on {path}; retrying in {wait:.0f}s")
                time.sleep(wait)
                continue
...
def get_all_markets():
    ...
    for series in ALL_SERIES:
        time.sleep(KALSHI_REQUEST_SPACING_SEC)  # respect rate limits
...
def analyze(markets, news, learning, balance, max_trade, open_positions_str):
    ...
    resp = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": (
                f"Current date/time: {now_utc}. Judge market close times "
                f"against this. "
